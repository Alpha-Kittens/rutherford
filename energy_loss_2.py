...
p = Result(0.0160708023688421, sys = 0.00027276731071674274) # dx = 1e-6 cm
                                                                # error is actually total. "sys" used for asymmetric evaluate
N = Result(0.8836819337898362, sys = 0.003197833742347611)
...
def do_poly_fit(incident, exiting1, exiting2, c_a, c_b, ps1, ps2):
    ...
    p1 = lambda p : evaluate_poly(E1, p)
    p2 = lambda p : evaluate_poly(E2, p)
...
def calculate_p():
    ...
    def get_N(p):
        ...
        for foil in foils:
            ...
            Ns.append(ps[foil] / evaluate_poly(E, p))
        wa = weighted_average(Ns)[0]
        print (wa)
        return wa
